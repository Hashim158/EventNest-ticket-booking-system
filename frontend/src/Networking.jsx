...
function Networking() {
  ...
  const fetchEvents = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        console.error("Failed to fetch Networking events:", res.status);
        return;
      }
      const events= await res.json();
      const sportsEvents = events.filter(event => event.category === "networking").sort((a, b) =>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {events.length === 0 ? (
          <p className="text-gray-600">No Networking events found.</p>
        ) : (
